	...
	<div id="page-content" class="single-page">
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-12">
					<ul class="breadcrumb">
						...
						<li><a style="text-align:center">Confirm a order</a></li>
					</ul>
				</div>
			</div>
			...
			<div class="col-lg-6">
			 ...
			 <label>Tên customer : <?php echo  $_SESSION['HoTen']?></label>
			 <label>Điện thoại: <?php echo  $_SESSION['dienthoai']?></label>
			 <label>Email:<?php echo    $_SESSION['email']?></label>     
			 <label><input type="text"  class="form-control" placeholder="Enter a address to delivery   :" name="address"  required ></label>
			 <br/>

			<label><input type="date" class="form-control" placeholder="Delivery date  :" name="date" id="datechoose"  required ></label>
			<label> Payment:<select class="selectpicker" name="hinhthuctt">
			...
			<label>service</label>
                    <select class="form-control select2" multiple="multiple" name="service" id="dichvu" onchange ="laygiatheoiddichvu(this.value)"  data-placeholder="Choose service" >
										<?php
			...
			<?php 
				 $total +=$_SESSION['cart'][$s["ID"]] * $s["Gia"]?>
				 	<?php 
							...
							<input type="submit" name="Order" value="Đặt hàng" class="btn btn-1" />	
			</div>
	</div>	
    </form>		
	<?php 
