	...
	<div id="page-content" class="single-page">
		<div class="container">
			...
			$ok=1;
			 ...
			 if($ok == 2)
			 {
				echo "Có ".count($_SESSION['cart']). " Have toys in cart ";
			 }
			else
			{
				echo   "<p>Haven't any toy in cart.</p>";
			}
					...
					<div class="col-md-9">
						<div class="caption">
							...
							<input class="form-inline quantity" style="margin-right: 80px;width:50px" min="1" max ="99" type="number" name ="qty[<?php echo $s["ID"] ?>]" value="<?php echo $_SESSION['cart'][$s["ID"]]?>"> 
						     <div>
								<input type="submit" name="update" style="margin-top:31px"  value="Update a toy" class="btn btn-2" />
							</div>
							<hr>
							<input type="submit" name="remove" value="Delete a toy" class="btn btn-default pull-right" />	
							<input type="hidden" name="idsprm" value="<?php echo $s["ID"] ?>" />
							<?php
                                 if($s["KhuyenMai"] == true)
								 {                                      
								?>
									<label style="color:red">Total price: <?php ;
			...
			<div class="row">
			<a href="rmcart.php" class="btn btn-2" style="margin-bottom:31px">Delete all cart</a>
